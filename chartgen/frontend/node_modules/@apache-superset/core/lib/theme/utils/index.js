 ...
 */
export function isSerializableConfig(config) {
    const { algorithm } = config;
    if (algorithm === undefined)
        return true;
    if (Array.isArray(algorithm))
        return algorithm.every(alg => typeof alg === 'string');
    return typeof algorithm === 'string';
}
/**
 * Converts a serializable theme config to an Ant Design ThemeConfig
 */
export function deserializeThemeConfig(config) {
...
export function serializeThemeConfig(config) {
    ...
    if (Array.isArray(algorithm)) {
        ...
        if (algorithm === antdThemeImport.defaultAlgorithm)
            ...
            serializedAlgorithm = ThemeAlgorithm.DEFAULT; // Fallback
    }
    return {
        ...rest,
 ...
 */
export function getAntdConfig(seed, isDark) {
 ...
 */
export function getSystemColors(antdTokens) {
    return {
